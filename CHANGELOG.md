...
- `ai` 서브커맨드 실행 시 대형 모델의 긴 응답 시간 동안 진행 상황을 직관적으로 확인할 수 있도록 stderr 전용 실시간 로딩 스피너 및 단계별 경과 시간(`[Xs]`) 표시 추가.

### Changed

- LLM HTTP 요청 기본 타임아웃을 120초에서 180초로 완화하고, 타임아웃 시 경량 모델 설정 방법을 안내하도록 메시지 개선.
